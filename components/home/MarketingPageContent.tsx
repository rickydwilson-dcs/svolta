...
type AuthState = 'anonymous' | 'free' | 'free-exhausted' | 'pro';

function useAuthState(): AuthState {
  const user = useUserStore((s) => s.user);
  const { isPro, canExport } = useUsageLimit();

  if (!user) return 'anonymous';
  if (isPro) return 'pro';
  if (!canExport) return 'free-exhausted';
  return 'free';
}

function HeroSection({ authState }: { authState: AuthState }) {
  if (authState === 'pro') {
    return (
      <section className="relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-hero-glow overflow-hidden">
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight mb-8 animate-slide-up">
            Welcome back
          </h1>
          <Link
            href="/editor"
...
function FinalCTASection({ variant }: { variant?: 'free' | 'exhausted' }) {
    ...
    <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-canvas">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <Link
          ...
          className="btn-pill btn-primary px-8 animate-pulse-glow"
        >
          {variant === 'free' ? 'Open Editor' : "Start Creating — It's Free"}
        </Link>
      </div>
    </section>
  );
...
export function MarketingPageContent() {
  ...
  return (
    <>
      {/* Hero — always shown, CTAs vary */}
      <HeroSection authState={authState} />

      {/* Features — anonymous + free only */}
      {(authState === 'anonymous' || authState === 'free') && (
        <FeaturesSection />
      )}
      ...
      {(authState === 'free' || authState === 'free-exhausted') && (
        <UsageIndicator />
      )}
      ...
      {/* Upgrade Nudge — free users only (replaces pricing) */}
      {authState === 'free' && (
        <UpgradeNudge />
      )}
      {authState === 'free-exhausted' && (
      ...
      {/* Install App Banner — all states (self-hides in PWA) */}
      <InstallAppBanner />

      {/* Final CTA — anonymous + free + free-exhausted only */}
      {authState === 'anonymous' && (
        <FinalCTASection />
      )}
      {authState === 'free' && (
        <FinalCTASection variant="free" />
      )}
      {authState === 'free-exhausted' && (
        <FinalCTASection variant="exhausted" />
      )}
    </>
  );
}
